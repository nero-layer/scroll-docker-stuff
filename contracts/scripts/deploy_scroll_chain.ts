...
async function main() {
  ...
  const MAX_TX_IN_BATCH = 25; // process.env.MAX_TX_IN_BATCH || 
  const PADDING_TX_HASH ="0xb5baa665b2664c3bfed7eb46e00ebc110ecf2ebd257854a9bf2b9dbc9b2c08f6";
    // process.env.PADDING_TX_HASH || 
  ...
  if (!addressFile.get("ScrollChain.implementation")) {
    ...
    const impl = await ScrollChain.deploy(CHAIN_ID_L2); // MAX_TX_IN_BATCH // process.env.MAX_TX_IN_BATCH || 
